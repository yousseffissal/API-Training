...
//this part dosn't work cus we need to install the cors package and import it in this file to use it, but i will let it here for me to see how to use it if I want to allow requests only from the frontend URL that is running on the port 8090, but if I want to allow requests from any origin you can just use app.use(cors()) without any options, if we don't use CORS middleware, the frontend will not be able to make requests to the backend if they are running on different ports, and we will get a CORS error in the browser console.to install the cors package, I can run the following command in my terminal : npm install cors in the backend or the frontend depending on where I want to use it, but in this case we need to install it in the backend because we want to allow requests from the frontend to the backend :
